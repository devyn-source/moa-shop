...
type ZoneMode = "move" | "nw" | "ne" | "se" | "sw";
...
export function StudioPreview({
  ...
  const zdrag = useRef<null | { mode: ZoneMode; id: string; startPx: { x: number; y: number }; startBox: Box }>(null);
  ...
  function selectLocation(loc: Loc) {
    ...
    setPlacements((prev) =>
      prev[key]
        ? prev
        : { ...prev, [key]: { art: SAMPLE_ART, aspect: 90 / 240, x: (loc.box.x0 + loc.box.x1) / 2, y: (loc.box.y0 + loc.box.y1) / 2, scale: loc.size, rot: 0, blend: "screen" } }
    );
  }
  ...
  function zDown(m: ZoneMode, id: string) {
    return (e: React.PointerEvent) => {
      ...
      if (!z) return;
      zdrag.current = { mode: m, id, startPx: ptrPct(e), startBox: { ...z.box } };
      e.currentTarget.setPointerCapture(e.pointerId);
    };
  }
  function zMove(e: React.PointerEvent) {
    ...
    if (it.mode === "move") {
      ...
      const box = { ...b };
      if (it.mode.includes("w")) box.x0 = clamp(p.x, 0, b.x1 - MIN);
      if (it.mode.includes("e")) box.x1 = clamp(p.x, b.x0 + MIN, 100);
      if (it.mode.includes("n")) box.y0 = clamp(p.y, 0, b.y1 - MIN);
      if (it.mode.includes("s")) box.y1 = clamp(p.y, b.y0 + MIN, 100);
      ...
      <div>
        ...
        <div className="studio-stage" ref={stageRef}>
          ...
          {mode === "place" ? (
            <>
              {activeLoc ? (
                <div className="studio-locbox" style={{ left: `${activeLoc.box.x0}%`, top: `${activeLoc.box.y0}%`, width: `${activeLoc.box.x1 - activeLoc.box.x0}%`, height: `${activeLoc.box.y1 - activeLoc.box.y0}%` }}>
            ...
            zoneConfig[view].map((z) => {
              ...
              return (
                <div key={z.id} className={`studio-azone${isActive ? " studio-azone--active" : ""}`} style={{ left: `${z.box.x0}%`, top: `${z.box.y0}%`, width: `${z.box.x1 - z.box.x0}%`, height: `${z.box.y1 - z.box.y0}%` }} onPointerDown={zDown("move", z.id)} onPointerMove={zMove} onPointerUp={endDrag}>
              ...
              {editingZone ? (
                <>
                  ...
                  <p className="trust-note" style={{ marginTop: 8 }}>
                    Box: {Math.round(editingZone.box.x0)},{Math.round(editingZone.box.y0)} → {Math.round(editingZone.box.x1)},{Math.round(editingZone.box.y1)}
                  </p>
                </>
              ) : (
