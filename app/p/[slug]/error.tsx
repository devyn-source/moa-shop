...
// PDP error boundary — branded recovery instead of Next's default screen.
...
export default function ProductError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="page" style={{ maxWidth: 560, textAlign: "center", paddingTop: 80 }}>
      ...
      <h1 className="page-title">This product didn&apos;t load</h1>
      <p className="lede" style={{ marginTop: 12 }}>
        Something went off-script rendering this style. Your cart and any saved configuration are safe — try again, or head back to the catalog.
      </p>
      ...
      <p className="trust-note" style={{ marginTop: 22 }}>
        Still stuck? <a href="mailto:production@magnumopus.agency" style={{ color: "var(--color-terracotta)", fontWeight: 600 }}>Email a real person</a> — include the product name and we&apos;ll sort it.
      </p>
    </main>
  );
}
